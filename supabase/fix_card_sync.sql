...
begin
  ...
  -- Ya está en cartón nuevo → éxito idempotente
...
-- add_stamp: si el cartón está completo, empieza uno nuevo y suma el punto
...
begin
  ...
  -- Cartón completo (gratis pendiente/canjeado): al sumar, arranca cartón nuevo
  if v_card.stamps_count >= v_cafe.stamps_required then
...
-- approve NFC: misma lógica (si estaba completo, nuevo cartón + 1 sello)
